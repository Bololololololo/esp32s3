...
TEST_CASE("MessageRouter: Concurrent Publishing from Multiple Threads", "[message-router-multithreading]") {
    ...
    MessageRouter::getInstance()->subscribe(ComponentId::COMPONENT_ID_TEST_PUB, subscriber);
    ...
    auto publishMessages = [&](int threadId) {
        for (int i = 0; i < MESSAGES_PER_THREAD; i++) {
            ...
            esp_err_t ret = MessageRouter::getInstance()->publish({ComponentId::COMPONENT_ID_TEST_PUB, payload.c_str()});
            CHECK(ret == ESP_OK);
            messageCount++;
        }
    };
    ...
    MessageRouter::getInstance()->unsubscribe(ComponentId::COMPONENT_ID_TEST_PUB, subscriber);
}
...
TEST_CASE("MessageRouter: Concurrent Subscribe/Unsubscribe Operations", "[message-router-multithreading]") {
    ...
    auto subscribeThread = [&]() {
        for (int i = 0; i < NUM_OPERATIONS; i++) {
            ...
            esp_err_t ret = MessageRouter::getInstance()->subscribe(ComponentId::COMPONENT_ID_TEST_PUB, 
                                                                     subscribers[subIndex]);
            CHECK(ret == ESP_OK);
            subscriptionCount++;
        }
    };
    ...
    for (const auto &sub : subscribers) {
        MessageRouter::getInstance()->unsubscribe(ComponentId::COMPONENT_ID_TEST_PUB, sub);
    }
}
...
TEST_CASE("MessageRouter: Concurrent Publish and Subscribe", "[message-router-multithreading]") {
    ...
    auto subscriberThread = [&]() {
        ...
        MessageRouter::getInstance()->subscribe(ComponentId::COMPONENT_ID_TEST_PUB, subscriber1);
        vTaskDelay(pdMS_TO_TICKS(15));
        MessageRouter::getInstance()->subscribe(ComponentId::COMPONENT_ID_TEST_PUB, subscriber2);
        vTaskDelay(pdMS_TO_TICKS(20));
        MessageRouter::getInstance()->unsubscribe(ComponentId::COMPONENT_ID_TEST_PUB, subscriber1);
    };
    ...
    MessageRouter::getInstance()->unsubscribe(ComponentId::COMPONENT_ID_TEST_PUB, subscriber2);
}
...
TEST_CASE("MessageRouter: Multiple Subscribers Receive Messages Concurrently", "[message-router-multithreading]") {
    ...
    for (int i = 0; i < NUM_SUBSCRIBERS; i++) {
        ...
        MessageRouter::getInstance()->subscribe(ComponentId::COMPONENT_ID_TEST_PUB, sub);
    }
...
TEST_CASE("MessageRouter: Rapid Subscribe/Unsubscribe Cycles", "[message-router-multithreading]") {
    ...
    for (int cycle = 0; cycle < CYCLES; cycle++) {
        esp_err_t ret = MessageRouter::getInstance()->subscribe(ComponentId::COMPONENT_ID_TEST_PUB, subscriber);
        CHECK(ret == ESP_OK);
        ...
        ret = MessageRouter::getInstance()->unsubscribe(ComponentId::COMPONENT_ID_TEST_PUB, subscriber);
        CHECK(ret == ESP_OK);
...
TEST_CASE("MessageRouter: Publish While Modifying Subscriber List", "[message-router-multithreading]") {
    ...
    auto modifierThread = [&]() {
        for (int iter = 0; iter < NUM_ITERATIONS; iter++) {
            ...
            MessageRouter::getInstance()->subscribe(ComponentId::COMPONENT_ID_TEST_PUB, newSub);
            ...
            if (subscribers.size() > 1) {
                ...
                MessageRouter::getInstance()->unsubscribe(ComponentId::COMPONENT_ID_TEST_PUB, subscribers[removeIdx]);
