...
static int heart_rate_chr_access(uint16_t conn_handle, uint16_t attr_handle, struct ble_gatt_access_ctxt *ctxt,
                                 void *arg);
        ...
        if (attr_handle == led_chr_val_handle) {
            ...
            if (ctxt->om->om_len == 1) {
                ...
                if (ctxt->om->om_data[0]) {
                    statusLed->setColor(utils::StatusLedColor::YELLOW);
                    ESP_LOGI(TAG, "led turned on!");
                } else {
                    statusLed->setColor(utils::StatusLedColor::OFF);
